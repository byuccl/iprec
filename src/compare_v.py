...
def compare_vertex(mapping,g1,v1,g2,v2,depth,verbose):
    ...
    if is_constant_vertex(v1):
        if verbose: print("\t"*depth,"\tIS CONST SOURCE")
    ...
    if compare_ref(v1,v2):
        if verbose: print("\t"*depth,"\tCELLS ARE THE SAME")
        # FOR EVERY SOURCE IN VERTEX
        edge_dict = get_edge_dict_in(v1,v2)
        if verbose: print("\t"*depth,"\t\tEDGES:",edge_dict)
        for k in edge_dict:
            for e2_source in edge_dict[k]["e2"]:
                es1 = edge_dict[k]["e1"]
                if len(es1) == 0: # NO MATCHING EDGES
                    if verbose: print("\t"*depth,"\t\tNO MATCHING EDGES",es1,e2_source,k)
                    return 0
                elif len(es1) == 1:  # ONLY ONE MATCHING EDGE
                    e1_source = es1[0]
                    if e1_source not in mapping:
                        if verbose: print("\t"*depth,"\t\tSOURCE NOT MAPPED")
                        if e2_source in mapping.values():
                            if verbose: print("\t"*depth,"\t\tSOURCE ALREADY HAS A MATCH")
                            return 0
                        mapping[e1_source] = e2_source
                        tmp_map = compare_vertex(dict(mapping),g1,g1.vs[e1_source],g2,g2.vs[e2_source],depth,verbose)
                        if tmp_map == 0:
                        ...
                        if verbose: print("\t"*depth,"\t\tSOURCE IS MAPPED")
                        if mapping[e1_source] != e2_source:
                            if verbose: print("\t"*depth,"\t\tSOURCE MAP MISMATCH")
                            return 0
                        else:
                            edge_dict[k]["e1"].remove(e1_source)
                    ...
                    if pass_flag == 0:
                        if len(possible_matches) == 0:
                            if verbose: print("\t"*depth,"\t\tNO SUCCESSFUL SOURCE MATCH")
                            return 0
                        elif len(possible_matches) == 1:
                            if verbose: print("\t"*depth,"\t\tONE SUCCESSFUL SOURCE MATCH")
                            #mapping[possible_matches[0]] = e2.source
                            mapping = list_maps[0][2]
                        else:
                            if verbose: print("\t"*depth,"\t\tMULTIPLE SUCCESSFUL SOURCE MATCH",possible_matches,list_maps)
                            # IF MULTIPLE MATCH, PICK THE FIRST ONE FOR NOW, MAY NEED A "SWAP PORTS" METHOD
                            mapping = list_maps[0][2]
                    else:
                        if verbose: print("\t"*depth,"\t\tONE PRE-FOUND SUCCESSFUL SOURCE MATCH")  
        #if verbose: print("\t"*depth,"\tMAPPING AFTER INPUTS",mapping)  

        edge_dict = get_edge_dict_out(v1,v2)
        if verbose: print("\t"*depth,"\t\tEDGES OUT:",edge_dict)
        # OUTPUTS
        for k in edge_dict:
            if verbose: print("\t"*depth,"\t\tK:",k)  
            for e2_target in edge_dict[k]["e2"]:
                es1 = edge_dict[k]["e1"]
                if len(es1) == 0: # NO MATCHING EDGES
                    if verbose: print("\t"*depth,"\t\tRET 1")  
                    return 0
                elif len(es1) == 1:  # ONLY ONE MATCHING EDGE
                    e1_target = es1[0]
                    if e1_target not in mapping:
                        if verbose: print("\t"*depth,"\t\tTARGET NOT MAPPED")
                        if e2_target in mapping.values():
                            key_val = list(mapping.keys())[list(mapping.values()).index(e2_target)]
                            if verbose: print("\t"*depth,"\t\tRET 2")  
                            return 0
                        mapping[e1_target] = e2_target
                        tmp_map = compare_vertex(dict(mapping),g1,g1.vs[e1_target],g2,g2.vs[e2_target],depth,verbose)
                        if tmp_map == 0:
                            mapping.pop(e1_target,None)
                            if verbose: print("\t"*depth,"\t\tRET 3")  
                            return 0
                        else:
                            edge_dict[k]["e1"].remove(e1_target)
                            mapping = tmp_map
                    else:
                        if verbose: print("\t"*depth,"\t\tTARGET IS MAPPED")
                        if mapping[e1_target] != e2_target:
                            if verbose: print("\t"*depth,"\t\tTARGET MAP MISMATCH")
                            if verbose: print("\t"*depth,"\t\tRET 4")  
                            return 0
                        else:
                            edge_dict[k]["e1"].remove(e1_target)
                else: # MULTIPLE MATCHING EDGES
                    if verbose: print("\t"*depth,"\t\tMULTIPLE EDGE MATCH:",v1.index,e2_target)
                    pass_flag = 0 
                    possible_matches = []
                    list_maps = []
                    for e1_target in es1:
                        if verbose: print("\t"*depth,"\t\tTESTING OUTPUT EDGE:",e1_target)
                        if e1_target not in mapping:
                            if verbose: print("\t"*depth,"\t\tTARGET NOT MAPPED")
                            if e2_target in mapping.values():
                                if verbose: print("\t"*depth,"\t\tTARGET ALREADY HAS A MATCH")
                                continue
                            mapping[e1_target] = e2_target
                            tmp_map = compare_vertex(dict(mapping),g1,g1.vs[e1_target],g2,g2.vs[e2_target],depth,verbose)
                            ...
                            if verbose: print("\t"*depth,"\t\tTARGET IS MAPPED")
                            if mapping[e1_target] != e2_target:
                                if verbose: print("\t"*depth,"\t\tTARGET MAP MISMATCH",e1_target,g1.vs[e1_target]["CELL_NAME"],mapping[e1_target],e2_target)
                                #if v1.index == 37:
                                #    print("E2 EDGES:")
                    ...
                    if pass_flag == 0:
                        if len(possible_matches) == 0:
                            if verbose: print("\t"*depth,"\t\tNO SUCCESSFUL TARGET MATCH")
                            return 0
                        elif len(possible_matches) == 1:
                            if verbose: print("\t"*depth,"\t\tONE SUCCESSFUL TARGET MATCH")
                            #mapping[possible_matches[0]] = e2_target
                            mapping = list_maps[0][2]
                        else:
                            if verbose: print("\t"*depth,"\t\tMULTIPLE SUCCESSFUL TARGET MATCH:", possible_matches,list_maps)
                            # IF MULTIPLE MATCH, PICK THE FIRST ONE FOR NOW, MAY NEED A "SWAP PORTS" METHOD
                            mapping = list_maps[0][2]
                            #mapping[possible_matches[0]] = e2_target
                            #mapping = compare_vertex(dict(mapping),g1,g1.vs[possible_matches[0]],g2,g2.vs[e2_target],depth)
                    else:
                        if verbose: print("\t"*depth,"\t\tONE PRE-FOUND SUCCESSFUL TARGET MATCH")  
        #if verbose: print("\t"*depth,"\tMAPPING AFTER OUTPUTS",mapping)  
    else:
        if verbose: print("\t"*depth,"\tRETURNING:CANDIDATES ARE NOT THE SAME",v1["ref"],v2["ref"])
        return 0
    return mapping
