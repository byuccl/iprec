...
# Creates the Library of Hierarchical Cell definitions for the randomized specimen designs for the IP
class LibraryGenerator():
    ...
    def import_design(self,design):
        ...
        # Create all edges
        nets = design["NETS"]
        edge_data = {"conns":[],"names":[],"parent":[],"in_pin":[],"out_pin":[],"signal":[]}
        for x in nets:
    ...
    def compare_templates(self,g1,template_file_name):
        ...
        self.print_graph(template_file_name.replace("library/" + self.ip + "/templates/","").replace(".pkl",""), g2)
        return 1
    ...
    def create_hier_cell(self,ref_name,g,user_properties):  
        ...
        version_count = len(os.listdir("library/" + self.ip + "/templates/" + ref_name + "/"))
        ...
        self.print_graph(ref_name + "/"+str(version_count), g)
        #cell_dict[ref_name + "/" + str(version_count)] = cell
        file_name = "library/" + self.ip + "/templates/" + ref_name + "/" + str(version_count) + ".pkl"
    ...
    def create_templates(self,g,templates):
        ...
        for v in g.vs.select(IS_PRIMITIVE=0):
            ...
            if v["ref"] not in templates:
                os.makedirs("library/" + self.ip + "/templates/"+v["ref"]+"/", exist_ok=True)
                os.makedirs("library/" + self.ip + "/graphs/"+v["ref"]+"/", exist_ok=True)
    ...
    def create_submodules(self):
        cell_graphs = os.listdir("data/" + self.ip + "/")
        cell_graphs[:] = [x for x in cell_graphs if ".json" in x and "properties" not in x]
        templates = {}
        for x in os.listdir("library/" + self.ip + "/templates/"):
            print("X:",x)
            templates[x] = []
            for y in os.listdir("library/" + self.ip + "/templates/" + x + "/"):
                templates[x] += ["library/" + self.ip + "/templates/" + x + "/" + y]
        for c in sorted(cell_graphs):
            fj = open("data/" + self.ip + "/" + c)
            try:
                design = json.load(fj) 
                fj.close()
    ...
    def init_templates(self):
        ...
        for x in os.listdir("library/" + self.ip + "/templates/"):
            ...
            for y in os.listdir("library/" + self.ip + "/templates/" + x + "/"):
                templates[x][y] = {}
                templates[x][y]["file"] = "library/" + self.ip + "/templates/" + x + "/" + y
        ...
        fj = open("library/" + self.ip + "/templates.json",'w')
    ...
    def print_graph(self,name,graph_obj):
        f = open("library/" + self.ip + "/graphs/" + name + ".txt","w")
...
# Runs the export design into a json for all specimen designs in parallel
def export_designs():
    global ip
